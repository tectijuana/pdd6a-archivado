...
//Lara Solís Angel Armando
//13210374
//Codigo ejemplo de PDD Flyweight
...
namespace EJERCICIO1
{
    ...
    {
        protected IDictionary<string, OpcionVehiculo> opciones = new Dictionary<string,
        OpcionVehiculo>();
        public OpcionVehiculo getOption(string nombre)
        {
            OpcionVehiculo resultado;
            if (opciones.ContainsKey(nombre)) resultado = opciones[nombre];
            else
            {
                resultado = new OpcionVehiculo(nombre); opciones.Add(nombre, resultado);
            }
            return resultado;
        }
    }
